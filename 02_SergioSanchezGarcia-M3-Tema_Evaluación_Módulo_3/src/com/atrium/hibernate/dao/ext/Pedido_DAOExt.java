...
@Scope("prototype")
public class Pedido_DAOExt extends PedidosDAO implements Serializable {
	...
	public Pedidos consultar_ConLinea(Integer numeroPedido) {
		...
		// 3. Resolucion de la relacion con los Usuario 
	...
	public List<Pedidos> consultar_Todo(String nombreUsuario) {
		// 1. Apertura de la consulta
		Criteria criteria = getCurrentSession().createCriteria(Pedidos.class);
		// 2. JOIN entre las tablas para resolver la relación
		criteria.setFetchMode("lineaPedido", FetchMode.JOIN);
		...
		criteria.setFetchMode("usuarios", FetchMode.JOIN);
		// 4. Clave primaria
		criteria.add(Restrictions.eq("usuarios.codUsuario", nombreUsuario));
	...
	public List<Pedidos> consultar_ConClientes() {
		...
		consulta.setFetchMode("usuarios", FetchMode.JOIN);
